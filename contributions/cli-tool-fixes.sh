...
REPO="$GITHUB_USER/cli-tool"
DIR="$REPOS_DIR/cli-tool"
...
git diff --staged --quiet || (git commit -m "Update cli-tool: $(date '+%Y-%m-%d %H:%M') [skip ci]" && git push origin HEAD 2>&1 | tee -a "$LOG_FILE" && log "cli-tool: Updated and pushed.") || log "cli-tool: No changes."
