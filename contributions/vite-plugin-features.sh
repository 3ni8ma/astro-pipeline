...
REPO="$GITHUB_USER/vite-plugin"
DIR="$REPOS_DIR/vite-plugin"
...
npx tsc --noEmit 2>&1 | tee -a "$LOG_FILE" || log "vite-plugin: TypeScript check had warnings."

git add src/index.ts
git diff --staged --quiet || (git commit -m "Update vite-plugin: $(date '+%Y-%m-%d %H:%M') [skip ci]" && git push origin HEAD 2>&1 | tee -a "$LOG_FILE" && log "vite-plugin: Updated and pushed.") || log "vite-plugin: No changes."
